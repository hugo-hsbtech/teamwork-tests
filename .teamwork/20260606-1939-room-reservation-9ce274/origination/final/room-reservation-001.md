...
|---|---|---|---|
| v1 | 2026-06-06 | Originação rascunhada | Captura inicial via origination-brainstorm; seções de captura preenchidas a partir do ledger [Q001](#q001)-[Q006](#q006). |
| v2 | 2026-06-06 | Atualização pós-checkpoint | Seções `originator`, `urgency`, `constraints` e `impact` atualizadas com respostas [Q007](#q007)-[Q010](#q010). Conflito de eixo-imagem resolvido. Metadado Originador atualizado. |
| v3 | 2026-06-06 | Seções derivadas recompostas pelo Sintetizador | `readiness` atualizado para ~74% (urgência respondida [Q008](#q008), reach 76, originator 80). `triage` recomposto: decisão Descoberta direcionada, confiança ~66, nota de tensão atualizada. `cto_escalation` promovida a SIM por integração com agenda confirmada ([Q009](#q009)). `handoff` e `discovery` alinhados à nova triagem. |
| v4 | 2026-06-06 | Confirmação humana da triagem ([Q011](#q011)) | Decisão de roteamento confirmada pelo solicitante em 2026-06-06 ([D001](#d001)/[D002](#d002)). Banner ⚠️ substituído por confirmação ✅ na triagem e no encaminhamento. Metadados de triagem preenchidos. Confiança do alcance alinhada de 75 para 76 (reconciliação cosmética). |
...
O custo do conflito de salas tem quatro eixos, todos confirmados pelo solicitante. Nenhuma dimensão foi quantificada numericamente; todos os valores abaixo são estimativas qualitativas a validar: ([Q006](#q006) + [Q010](#q010))
...
A urgência é **pressão contínua**, confirmado diretamente pelo solicitante ([Q008](#q008)). Não há gatilho específico, prazo ou janela de oportunidade que motive a resolução agora: o que move é a persistência crônica do problema (frequência semanal confirmada em [Q003](#q003)) e o acúmulo de reclamações ao longo do tempo. A ausência de deadline é ela mesma a resposta. O custo de esperar é concreto: cada semana sem solução representa novos conflitos de agendamento, mais retrabalho operacional e mais reuniões importantes prejudicadas.
...
Situação: Triado. Decisão firmada pelo solicitante ([Q011](#q011), 2026-06-06). Referências ao ledger de decisões da iniciativa: [D001](#d001) (Descoberta direcionada aprovada) e [D002](#d002) (Escalação arquitetural confirmada).

### Critérios de triagem

| Critério | Avaliação |
|---|---|
| **Clareza do problema** | Forte: conflito de agendamento (double-booking) com sintomas observáveis e recorrência semanal (problem 80, [Q001](#q001)/[Q003](#q003)) |
| **Alcance** | Forte: quatro perfis, incluindo externos (imagem) e diretoria (reach 76, [Q004](#q004)) |
| **Impacto de negócio** | Fraco / não quantificado: quatro eixos qualitativos, nenhum valor quantificado; marcado **premissa** (impact 65, [Q006](#q006)/[Q010](#q010)). Item genuíno de descoberta. |
| **Urgência: por que agora** | Respondida: pressão contínua confirmada pelo solicitante (urgency 70, [Q008](#q008)); sem prazo específico, mas não mais inferida. |
| **Maturidade das premissas** | Aberta: 3 premissas materiais a validar + 1 incógnita técnica CONFIRMADA: viabilidade da integração com agenda corporativa (Google/Outlook, [Q009](#q009)). |
...
| **Justificativa** | Quadro mais forte (problema, alcance, originador, urgência sólidos), mas persistem 2 itens genuínos de descoberta que impedem congelar escopo: (1) impacto não quantificado (premissa de VALOR) e (2) viabilidade da integração com agenda corporativa, agora requisito CONFIRMADO ([Q009](#q009)) e portanto incógnita técnica/arquitetural real. Decisão fica em Descoberta, estreitada a esses dois eixos, em vez de promover a "Pronto para Produto" prematuramente. |
...
|---|---|---|
| A solução **precisa integrar com a agenda corporativa** (Google Calendar e/ou Outlook) | Técnica | Restrição firme declarada pelo solicitante ([Q009](#q009)). Desambiguação pendente: uma plataforma ou ambas? Há requisitos de SSO ou acesso a APIs corporativas? Detalhes de escopo, sem bloquear o gate de originação. |

Fora a integração com agenda corporativa, nenhuma outra restrição foi declarada: sem orçamento definido, sem prazo e sem restrições de complexidade informadas ([Q009](#q009)).
...
| Viabilidade da integração com agenda corporativa (Google Calendar / Outlook) | Requisito CONFIRMADO ([Q009](#q009)): gatilho de escalação arquitetural; definir plataforma(s), APIs corporativas, SSO/OAuth, modelo de sincronização | Revisão de viabilidade/arquitetura com Tech Lead/CTO + TI | 1 semana | (em aberto) |
...
> Nota: linha de urgência encerrada, respondida em [Q008](#q008). Integração com agenda corporativa promovida de "possível gatilho" a requisito confirmado a investigar.
...
| 3 | Viabilidade da integracao com agenda corporativa (Google/Outlook): plataforma(s), APIs, SSO/OAuth, sincronizacao | Requisito CONFIRMADO ([Q009](#q009)) · **Gatilho de escalacao arquitetural ao CTO** | Tech Lead / CTO + TI | 1 semana | Critica |
...
- **Escalação arquitetural (CTO / Tech Lead):** acionar revisão de viabilidade/arquitetura da integração com agenda corporativa (Google/Outlook), requisito confirmado ([Q009](#q009)), antes do congelamento de escopo.
...
- **Confiança:** 72
- **Disposição:** respondida
- **Alimenta:** Seção Restrições; gatilho de escalação arquitetural ao CTO ([D002](#d002))
...
- **Alimenta:** Seção Triagem (ver [D001](#d001)/[D002](#d002))
...
|---|---|---|---|---|---|
| Problema | 80 | Solicitante direto ([Q001](#q001) + [Q003](#q003)) | respondida | respondida | Dor de conflito de agendamento (double-booking) confirmada com frequência semanal crônica, atingindo o min-confidence=80 da seção. Ainda sem quantificação de horas perdidas por evento. Frequência estabelece severidade como contínua. |
| Originador e contexto | 80 | Solicitante direto ([Q005](#q005) + [Q007](#q007)) | respondida | respondida | Papel do solicitante confirmado em [Q007](#q007) como canalizador da demanda coletiva (opção B, "Fui eu + outros reclamantes"). A ausência de nome próprio não compromete o gate: origem confirmada ([Q005](#q005)), papel confirmado ([Q007](#q007)), pressão coletiva documentada. Demanda bottom-up; peso institucional moderado; pode demandar mais validação de partes interessadas para priorização formal. |
| Quem é impactado (alcance) | 76 | Solicitante direto ([Q004](#q004)) | respondida | respondida | Todas as quatro opções substantivas selecionadas pelo solicitante. A presença de reuniões com externos (imagem) e liderança/diretoria distingue esta dor de um problema puramente operacional interno. Seção atinge min-confidence=70. (Confiança ajustada de 75 para 76 em reconciliação cosmética com os valores citados na maturidade e na triagem.) |
| Impacto de negócio | 65 | Solicitante direto ([Q006](#q006) + [Q010](#q010)) | respondida | premissa | Impacto qualitativo confirmado em quatro eixos; conflito de eixo-imagem RESOLVIDO via [Q010](#q010): custo de imagem/relacionamento está subsumido em "negócios adiados", não é eixo à parte. Impacto permanece sem quantificação (único bloqueador residual do gate). Para firmar a confiança: (1) coletar nº de conflitos/semana × minutos perdidos por evento; (2) exemplos concretos de negócios adiados e valor estimado em receita ou relacionamento. "Negócios adiados" é a dimensão de maior gravidade; priorizar sua validação. |
| Urgência: por que agora | 70 | Solicitante direto ([Q008](#q008)) | respondida | respondida | Urgência perguntada diretamente em [Q008](#q008); solicitante escolheu opção A ("Pressão contínua"), substantiva, não escape-hatch. A natureza da urgência está estabelecida. Intensidade relativa dessa pressão frente a outras prioridades institucionais não foi capturada: não há como saber se o item está no topo da fila ou no backlog. Coerente com [Q005](#q005) (reclamação recorrente) e [Q003](#q003) (frequência semanal crônica). |
