...
- **Observação:** Dor de double-booking confirmada com frequência semanal crônica — atinge o min-confidence=80 da seção. Ainda sem quantificação de horas perdidas por evento. Frequência estabelece severidade como contínua.
...
- **Quem organiza o escritório (admin / facilities / recepção)** — absorve o retrabalho de remarcar, negociar e "apagar incêndio" a cada ocorrência.
...
O custo do conflito de salas é multifacetado, com quatro eixos confirmados pelo Submitter. Nenhuma dimensão foi quantificada numericamente — todos os valores abaixo são estimativas qualitativas a validar: (Q006 + Q010)
...
- **Negócios adiados (receita / relacionamento):** reuniões de negócio que não acontecem no horário previsto empurram decisões e fechamentos para frente, gerando custo de oportunidade em receita e relacionamento — **dimensão de maior gravidade potencial**. O custo de imagem/relacionamento com clientes e externos está **subsumido** neste eixo (confirmado pelo Submitter em Q010 — não é dimensão separada).
...
A urgência é **pressão contínua** — confirmado diretamente pelo Submitter (Q008). Não há gatilho específico, prazo ou janela de oportunidade que motive a resolução agora: o driver é a persistência crônica do problema (frequência semanal confirmada em Q003) e o acúmulo de reclamações ao longo do tempo. A ausência de deadline é ela mesma a resposta. O custo de esperar é concreto: cada semana sem solução representa novos conflitos de agendamento, mais retrabalho operacional e mais reuniões importantes prejudicadas.
...
> Sinal leve que **semeia** a classificação greenfield/brownfield do PO na triagem (não exige certeza). Esta demanda parece ser uma **nova capacidade** (algo que não existe hoje) ou uma mudança em **algo que já funciona**? O PO firma a classificação formal no Registro de Intake.
...
- **Situação:** respondida
- **Disposição:** inferida
- **Observação:** Inferência a partir da ausência de qualquer menção a sistema atual. O PO deve confirmar se há alguma ferramenta informal (planilha, quadro, calendário compartilhado) em uso antes de classificar formalmente como greenfield.
...
| Critério | Avaliação |
|---|---|
| **Clareza do problema** | Forte — double-booking com sintomas observáveis e recorrência semanal (problem 80, Q001/Q003) |
...
| **Urgência — por que agora** | Respondida — pressão contínua confirmada pelo Submitter (urgency 70, Q008); sem prazo específico, mas não mais inferida. |
...
|---|---|---|
| A magnitude do impacto (tempo perdido, reuniões prejudicadas, retrabalho) é estimativa qualitativa, não quantificada | A validar | PO + quem organiza as salas (admin/facilities) — coletar nº de choques/semana e minutos por evento |
...
| Não existe nenhuma ferramenta formal de reserva de salas em uso hoje (greenfield) | A validar | PO + admin/facilities — confirmar se há planilha, quadro, calendário compartilhado ou qualquer processo informal em uso |
...
|---|---|---|
| A solução **precisa integrar com a agenda corporativa** (Google Calendar e/ou Outlook) | Técnica | Restrição firme declarada pelo Submitter (Q009). Desambiguação pendente: uma plataforma ou ambas? Há requisitos de SSO ou acesso a APIs corporativas? Detalhes de escopo — não bloqueadores do gate de originação. |
...
| Incógnita | Por que importa | Como investigar | Caixa-de-tempo | Resultado |
|---|---|---|---|---|
| Magnitude quantificada do impacto (nº choques/semana × min/evento) | Define se o valor justifica priorizar agora; hoje 100% qualitativo (impact 65, premissa) | Coletar registros / observação com admin / facilities / recepção | 1 semana | — |
| "Negócios adiados" é custo real de receita / relacionamento? | Eixo de maior gravidade potencial, não comprovado | Entrevistar liderança comercial / diretoria por exemplos concretos | 3 dias | — |
| Viabilidade da integração com agenda corporativa (Google Calendar / Outlook) | Requisito CONFIRMADO (Q009) — gatilho de escalação arquitetural; definir plataforma(s), APIs corporativas, SSO/OAuth, modelo de sincronização | Revisão de viabilidade/arquitetura com Tech Lead/CTO + TI | 1 semana | — |
| Existe ferramenta / processo informal hoje (greenfield vs. brownfield)? | Define natureza da demanda e escopo | Confirmar com PO + admin / facilities | 2 dias | — |

**Caixa-de-tempo total sugerida:** ~1 a 1,5 semana.
...
- **Colaboradores (admin / facilities / recepção):** coletar nº de choques/semana e minutos por evento.
- **Colaboradores (liderança comercial / diretoria):** confirmar e exemplificar "negócios adiados".
- **A confirmar (PO + admin / facilities):** verificar se já existe ferramenta / processo informal (planilha, quadro, calendário) antes de classificar como greenfield.
