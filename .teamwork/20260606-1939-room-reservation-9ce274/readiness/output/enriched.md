...
<!-- VISUAL (enrichment, additive): V05 - Metricas baseline-meta e impacto financeiro -->

**Impacto financeiro e metas operacionais** (dados de campo: D003/Q008 + D005/Q012)

| Metrica | Baseline (campo) | Meta pos-rollout | Prazo | Confianca |
|---|---|---|---|---|
| Conflitos de agendamento/semana | 1,8 | 0 | 4 semanas apos go-live | Alta |
| Adocao do sistema (% reservas no sistema) | 0% | 100% | 2 semanas apos go-live | Alta |
| Novos adiamentos de negocios por conflito | 9 em 5 semanas | 0 | 8 semanas apos go-live | Media |
| Pipeline total afetado (referencia) | ~R$ 700 mil | Protegido | -- | Alta |
| Perdas confirmadas (nao recuperaveis) | R$ 111 mil | -- | -- | Alta |
| Valor em risco (monitorar com D02) | R$ 90 mil | Recuperar | 8 semanas | Media |

> Guardrail: volume total de reservas/semana e carga de suporte da secretaria nao devem piorar em relacao ao baseline medido na semana pre-go-live.
...
<!-- VISUAL (enrichment, additive): V03 - Mapa de personas e JTBD -->

```mermaid
flowchart TD
    subgraph P1["Persona 1: Equipes internas"]
        J1["JTBD: Reservar sala com certeza\nde disponibilidade no horario"]
        D1["Hoje: double-booking descoberto\nna porta da sala"]
        A1["Pos-entrega: reserva confirmada\npelo sistema, zero conflito"]
    end

    subgraph P2["Persona 2: Reunioes com externos"]
        J2["JTBD: Conduzir reunioes com\nclientes sem imprevistos"]
        D2["Hoje: conflito gera\nconstrangimento institucional"]
        A2["Pos-entrega: reserva confirmada\nelimina risco de imagem"]
    end

    subgraph P3["Persona 3: Lideranca e diretoria"]
        J3["JTBD: Garantir que reunioes\nestrategicas acontecam no horario"]
        D3["Hoje: sem tratamento preferencial;\nsujeita ao mesmo double-booking"]
        A3["Pos-entrega: visibilidade em\ntempo real; anti-conflito automatico"]
    end

    subgraph P4["Persona 4: Secretaria / Facilities"]
        J4["JTBD: Manter processo de reservas\nordenado; resolver conflitos"]
        D4["Hoje: unico ponto de controle;\nabsorve todo o retrabalho"]
        A4["Pos-entrega: sistema assume arbitro;\nsecretaria = stakeholder-chave da migracao"]
    end

    SISTEMA["Sistema de Reserva de Salas"]

    P1 --> SISTEMA
    P2 --> SISTEMA
    P3 --> SISTEMA
    P4 -->|"stakeholder-chave da migracao"| SISTEMA
```
...
<!-- VISUAL (enrichment, additive): V04 - Tabela de escopo Incluido / Excluido / Adiado -->

**Resumo de escopo por disposicao**

| Disposicao | Item |
|---|---|
| **Incluido** | CRUD de reservas |
| **Incluido** | Integracao Google Calendar (uma via: sistema -> Calendar, <=2 min) |
| **Incluido** | Autenticacao simples (login social) |
| **Incluido** | Anti-double-booking automatico |
| **Incluido** | Reservas recorrentes (series, validacao por ocorrencia) |
| **Incluido** | Migracao e descontinuacao do processo manual (mural + Forms) |
| **Excluido** | Integracao com Outlook / Microsoft 365 |
| **Excluido** | SSO corporativo |
| **Excluido** | Outros recursos fisicos (estacionamento, equipamentos) |
| **Excluido** | Analytics avancado e relatorios de uso |
| **Excluido** | Portal externo para visitantes |
| **Adiado (Fase 2)** | Integracao com Outlook |
| **Adiado (Fase 2)** | Analytics e relatorios de ocupacao |
| **Adiado (Fase 2)** | Lembretes automaticos |
| **Adiado (Fase 3)** | SSO corporativo |
...
<!-- VISUAL (enrichment, additive): V02 - Maquina de estados da reserva -->
...
flowchart LR
    ...
    R -->|"Validacao anti-conflito: OK\n+ dispara sync Calendar"| C
    R -->|"Sobreposicao detectada\n(RN-01/02/03)"| E
    C -->|"Cancelamento pelo usuario\n(liberacao imediata do slot)"| CA
    C -->|"Fim do intervalo reservado\n(transicao automatica)"| CO
    CA -->|"Remove evento no Calendar\n+ notifica participantes"| CA
    E -->|"Exibe mensagem de conflito;\nnao cria reserva"| E

    style E fill:#fdd,stroke:#c00
    style C fill:#dfd,stroke:#080
    style CA fill:#ffe,stroke:#880
    style CO fill:#ddf,stroke:#008
```

<!-- VISUAL (enrichment, additive): V01 - Fluxo de criacao de reserva e deteccao/bloqueio de conflito -->

```mermaid
flowchart TD
    U1["Usuario seleciona\nsala e intervalo"]
    Q1{"Sistema consulta\nreservas ativas\nno intervalo"}
    BLOCK["Bloqueia confirmacao\nInforma conflito\n(sugere alternativas)"]
    ATOMIC["Checagem + gravacao\natomicas (first-commit-wins)"]
    CONFIRM["Reserva CONFIRMADA\n(RN-01/05)"]
    SYNC["Dispara sync\nGoogle Calendar\n(<=2 min, uma via)"]
    CALOK["Evento criado\nno Calendar"]
    CALFAIL["Reserva permanece CONFIRMADA\n(fonte de verdade)\nStatus: sync pendente\nRetry com backoff"]

    U1 --> Q1
    Q1 -->|"Intersecao detectada\n(sobreposicao parcial ou total)"| BLOCK
    Q1 -->|"Sem intersecao"| ATOMIC
    ATOMIC --> CONFIRM
    CONFIRM --> SYNC
    SYNC -->|"API disponivel"| CALOK
    SYNC -->|"API indisponivel\n(EC-02)"| CALFAIL

    style BLOCK fill:#fdd,stroke:#c00
    style CONFIRM fill:#dfd,stroke:#080
    style CALFAIL fill:#ffe,stroke:#880
```

<!-- VISUAL (enrichment, additive): V07 - Sincronizacao uma via sistema para Google Calendar -->

```mermaid
flowchart LR
    subgraph SISTEMA["Sistema (fonte de verdade)"]
        R1["Reserva CONFIRMADA"]
        R2["Reserva CANCELADA"]
    end

    subgraph CALENDAR["Google Calendar (espelho de leitura)"]
        C1["Evento criado\n(<=2 min)"]
        C2["Evento removido\n(<=2 min)"]
    end
    ...
    NOTA["Alteracoes feitas diretamente\nno Calendar NAO retroalimentam\no sistema neste release\n(EC-04 / EC-13)"]

    CALENDAR -.->|"sem retorno"| NOTA

    style NOTA fill:#ffe,stroke:#880
```
...
<!-- VISUAL (enrichment, additive): V06 - Jornada do usuario happy path como fluxo -->

```mermaid
flowchart TD
    START([Inicio: usuario quer\nreservar uma sala])
    A1["A1: Login social\n(conta Google)"]
    A2["A2: Consulta disponibilidade\nde salas em tempo real"]
    A3["A3: Escolhe sala, horario\ne preenche dados\n(titulo, participantes)"]
    A4["A4: Confirma a reserva"]
    Q2{"Conflito\ndetectado?"}
    BLOCKED["Sistema bloqueia\nInforma conflito\n(ALT-1)"]
    A5_SYS["A5: Sistema sincroniza\ncom Google Calendar\n(<=2 min)"]
    A6_SYS["A6: Participantes recebem\nconvite no Calendar"]
    ...
    A4 --> Q2
    Q2 -->|"Sim"| BLOCKED
    BLOCKED -->|"Usuario escolhe\noutra sala/horario"| A2
    Q2 -->|"Nao"| A5_SYS
    A5_SYS --> A6_SYS
    A6_SYS --> END

    style BLOCKED fill:#fdd,stroke:#c00
    style END fill:#dfd,stroke:#080
```
