# Avaliacao tecnica: sistema de reserva de salas de reuniao
...
| Versao | Data | Autor | Situacao | Resumo |
|---|---|---|---|---|
| v1 | 2026-06-07 | Doc Updater (Fase 2 - classificar & herdar) | Em andamento | DOC instanciado do template; tech-classification firmada (Hibrido, ambos os caminhos, KB inexistente a Discovery); secoes herdadas preenchidas (po-questions, integrations, affected-systems, nfr-feasibility, testability-observability, effort-cost) com Origin: inherited. |
...
| **Ressalvas** | R1: admin do Workspace permite grant OAuth a apps de terceiros (`calendar.events`), credencial/quotas confirmados (DISC-001). R2: first-commit-wins via exclusao transacional no banco (nao validacao read-then-write de aplicacao). R3: sync estritamente uma-via sistema a Calendar, sem reconciliacao reversa neste release. R4: revisao LGPD Juridico/DPO antes do release. |
...
### Resumo de viabilidade por NFR

| NFR | Categoria | Viavel? | Gated em Discovery? |
|---|---|---|---|
| NFR-01 [CRITICO] | Anti-double-booking / concorrencia | Sim | Nao |
| NFR-02 | Sync ≤2 min (Calendar) | Com ressalvas | Sim (DISC-001) |
| NFR-03 | Latencia ≤2s/≤3s p95 | Sim | Nao |
| NFR-04 [CRITICO] | Usabilidade (mais simples que Forms) | Com ressalvas | Nao (validacao de produto) |
| NFR-05 | Responsivo / mobile-first | Sim | Nao |
| NFR-06 | Login social (OAuth/OIDC) | Com ressalvas | Sim (DISC-001) |
| NFR-07 | LGPD basico | Com ressalvas | Nao (revisao DPO) |
| NFR-08 | Somente Google Calendar (sem Outlook) | Com ressalvas | Sim (DISC-001) |
| NFR-09 | Disponibilidade ≥99% horario comercial | Sim | Nao |
| NFR-10 | Suporte a ~200 usuarios | Sim | Nao |

```mermaid
pie title NFR-01..10: Distribuicao de viabilidade
    "Sim (sem ressalva)" : 5
    "Com ressalvas / gated Discovery" : 3
    "Com ressalvas / revisao externa" : 2
```
...
|---|---|---|---|
| **Google Calendar API** | Google Workspace (externo) | Sincrono na escrita, **uma via** (sistema a Calendar; Calendar e espelho read-only per C-Q02/RN-04) | **Medio**: terceiro fora de controle; quota/disponibilidade afetam o SLA ≤2 min (NFR-02). Quota real **a descobrir (spike)**. |
...
### Arquitetura-alvo (C4: Contexto + Container; sync UMA VIA sistema a Calendar)
...
flowchart TD
    ...
    subgraph Sistema["Sistema de Reserva de Salas (fonte de verdade)"]
        FE["Web Frontend\nReact responsivo\n(mobile ≥360px / tablet ≥768px)"]
        API["API / Backend\nNestJS/Express\n(REST/JSON)"]
        PG["PostgreSQL\nEXCLUSION constraint\ntstzrange + room_id"]
        WORKER["Worker de Sync Calendar\n(assincrono, retry/backoff)\ntransactional outbox"]
    end

    subgraph Externos["Servicos externos (Google)"]
...
> Direcao do sync: SISTEMA a Google Calendar (uma via). O Calendar nao retroalimenta o sistema neste release (ADR-003, NFR-02/08, C-Q02).
...
- **Estrategia de branching / CI:** trunk-based + PR com revisao; `main` sempre deployavel. CI: lint a build a unit/integracao (incl. teste de concorrencia NFR-01) a e2e; deploy continuo p/ homologacao + promocao manual; migracoes forward-only.
...
> **Contexto:** sync uma via (sistema a Calendar) ≤2 min; alteracoes no Calendar nao retroalimentam (RN-04, NFR-02/08, US-06, EC-02/04/13); login social, SSO/Outlook fora do escopo.
...
```mermaid
flowchart LR
    SIS["Sistema de Reserva\nde Salas"]

    subgraph Google["Google (externo)"]
        GCAL["Google Calendar API\ncalendar.events\nOAuth 2.0"]
        IDP["Provedor de Identidade\nOAuth2/OIDC\n(login social)"]
    end

    SIS -- "sync REST uma via\n(outbox + worker, ≤2 min)\nViavel com ressalvas\ngated DISC-001" --> GCAL
    SIS -- "autenticacao delegada\nOIDC login social\nViavel" --> IDP

    style GCAL fill:#fff3e0,stroke:#e65100
    style IDP fill:#e8f5e9,stroke:#388e3c
```
...
| Sincronizacao de agenda (sistema a Calendar, uma via) | **Reutilizar** | Google Calendar API REST (`calendar.events`); nao construir backend de calendario. | Baixo custo, dentro de quota gratuita (~200 usuarios); viabilidade depende de TA-Q01/Q02/Q04. |
...
| NFR-02 | Toda reserva confirmada ou cancelada deve refletir no Google Calendar em **≤2 minutos** (sync uma via: sistema a Calendar). | **Com ressalvas** | Worker assincrono (fila + retry/backoff) propaga ≤2 min (folgado p/ job assincrono); reserva persiste como fonte de verdade independentemente do sync. | **Gated na Discovery** (TA-Q01/Q04): quotas/rate-limits e modelo de credencial nao confirmados. |
...
| NFR-08 | O sistema interopera exclusivamente com a **API do Google Calendar** neste release (uma via: sistema a Calendar); integracao com Outlook esta fora do escopo. | **Com ressalvas** | Interoperacao so com Calendar API, uma via; Outlook fora. Reusa worker/credencial de NFR-02. | **Gated na Discovery** (TA-Q01); risco de scope creep se bidirecional reentrar (R05). |
...
### Tabela de viabilidade dos NFRs (visao rapida)

| NFR | Descricao resumida | Viavel? | Gate / Condicao |
|---|:---|:---:|---|
| NFR-01 [CRITICO] | Anti-double-booking (concorrencia) | **Sim** | Harness de concorrencia obrigatorio (gate de release) |
| NFR-02 | Sync ≤2 min a Calendar | **Com ressalvas** | DISC-001: quotas + credencial |
| NFR-03 | Latencia ≤2s consulta / ≤3s confirmacao p95 | **Sim** | Validar em pico de carga |
| NFR-04 [CRITICO] | Usabilidade (mais simples que Forms) | **Com ressalvas** | Validacao qualitativa com secretaria (produto) |
| NFR-05 | Responsivo mobile ≥360px / tablet ≥768px | **Sim** | Teste em dispositivo movel real |
| NFR-06 | Login social OAuth/OIDC | **Com ressalvas** | DISC-001: consent screen / escopos |
| NFR-07 | LGPD basico (retencao, exclusao) | **Com ressalvas** | Revisao Juridico/DPO (bloqueante pre-release) |
| NFR-08 | Somente Calendar API (sem Outlook) | **Com ressalvas** | DISC-001: credencial/quota |
| NFR-09 | Disponibilidade ≥99% horario comercial | **Sim** | Manutencao fora do horario |
| NFR-10 | Suporte ~200 usuarios (pico 10h-16h) | **Sim** | Teste de carga confirmatorio |
...
| **Sincronizacao uma-via (sistema a Calendar)** | Plataforma/Externo | Firmado C-Q02: sistema e fonte de verdade; Calendar e espelho; nao retroalimenta (EC-04/EC-13). | Sem reconciliacao bidirecional neste release; sem webhooks de entrada. |
...
| Divergencia silenciosa Calendar e sistema (sync uma-via) | Dados | Media | Medio | Aviso na UI (Calendar e espelho read-only, EC-04/EC-13); sistema e fonte unica de verdade (RN-04/C-Q02); sem reconciliacao reversa neste release. |
...
### Matriz de risco tecnico (probabilidade x impacto)

```mermaid
flowchart TD
    subgraph ALTO["Impacto ALTO"]
        R1["R1: Grant OAuth negado\n(Integracao)\nProb: Media"]
        R2["R2: Race condition\nanti-double-booking\n(Tecnica)\nProb: Media"]
    end
    subgraph MEDIO["Impacto MEDIO"]
        R3["R3: Quota/latencia\nCalendar API\n(Integracao)\nProb: Baixa-Media"]
        R4["R4: Divergencia silenciosa\nCalendar vs. sistema\n(Dados)\nProb: Media"]
        R5["R5: Tratamento\nfuso/DST\n(Dados)\nProb: Baixa"]
    ...
    style R6 fill:#e8f5e9,stroke:#388e3c
```

| Risco | Prob. | Impacto | Prioridade | Gate/Mitigacao-chave |
|---|---|---|---|---|
| R1: Grant OAuth Workspace | Media | Alto | CRITICO | DISC-001 (deve preceder commit de arquitetura) |
| R2: Race condition booking | Media | Alto | CRITICO | Harness de concorrencia (gate de release) |
| R3: Quota/latencia Calendar | Baixa-Media | Medio | Monitorar | Worker backoff + alerta de quota |
| R4: Divergencia silenciosa | Media | Medio | Monitorar | Aviso na UI; sistema e fonte de verdade |
| R5: Fuso/DST | Baixa | Medio | Baixo | UTC + bloqueio de intervalos ambiguos |
| R6: Falha de sync | Baixa | Medio | Baixo | Outbox + retry idempotente |
...
### ADRs: visao de referencia rapida

| ADR | Tema | Status | NFR/Restricao ancora | Bloqueia |
|---|---|---|---|---|
| ADR-001 | Persistencia relacional + EXCLUSION constraint anti-overlap | Aprovado ✓ | NFR-01 [CRITICO], RN-05 | Nada (fundacao) |
| ADR-002 | Sync assincrona via transactional outbox + worker | Aprovado ✓ | NFR-02, EC-02 | ADR-003 |
| ADR-003 | Sistema = fonte de verdade; Calendar = espelho read-only | Aprovado ✓ | NFR-08, C-Q02, RN-04 | ADR-002 |
| ADR-004 | Autenticacao OAuth2/OIDC (login social Google) | Aprovado ✓ | NFR-06, NFR-07 | ADR-006 (credencial Calendar) |
| ADR-005 | Timestamps em UTC + mono-fuso + bloqueio DST | Aprovado ✓ | EC-06, RN-09/10/11 | Nada |
| ADR-006 | Modelo de credencial Calendar (service account vs por usuario) | Gated DISC-001 | TA-Q01/Q05, D03 | Integracao Calendar (ADR-002/003) |
...
flowchart LR
    ...
    style ADR006 fill:#fff3e0,stroke:#e65100,stroke-dasharray: 5 5
    style ADR001 fill:#e8f5e9,stroke:#388e3c
    style ADR002 fill:#e8f5e9,stroke:#388e3c
    style ADR003 fill:#e8f5e9,stroke:#388e3c
    style ADR004 fill:#e8f5e9,stroke:#388e3c
    style ADR005 fill:#e8f5e9,stroke:#388e3c
```

---

## Avaliacao de esforco e custo
...
### Esforco de desenvolvimento (dias-pessoa; time pequeno)
...
```mermaid
xychart-beta
    title "Esforco de desenvolvimento por area (dias-pessoa)"
    x-axis ["Backend nucleo", "Anti-double-booking", "Integracao/OAuth", "Frontend", "QA", "LGPD/Migracao"]
...
O release cria uma fundacao reutilizavel (stack + tech-landscape semeado) para fases futuras: Outlook na Fase 2 reusa o worker e o modelo fonte-de-verdade a espelho; SSO na Fase 3 se apoia na camada de auth existente; analytics e lembretes na Fase 2 usam o modelo de reservas. O custo recorrente incremental e baixo; o release elimina o processo manual como divida operacional. O maior driver de longo prazo e adocao (R01/R02/R03, fora desta estimativa).

---

## Ciclo de vida da reserva (diagrama de estados)

> O diagrama abaixo representa o ciclo de vida de uma reserva conforme as regras de negocio (RN-04/05/13/14, EC-01/02, NFR-01/02) documentadas no TA. Falha de sync com o Calendar nao muda o estado da reserva (a reserva permanece Confirmada; o worker faz retry per ADR-002).

```mermaid
stateDiagram-v2
    [*] --> Rascunho : usuario inicia criacao

    Rascunho --> Confirmada : submit (validacao OK)\nfirst-commit-wins\n(EXCLUSION constraint, ADR-001)
    Rascunho --> [*] : usuario descarta

    Confirmada --> Cancelada : usuario cancela\n(ou admin cancela)
    Confirmada --> Concluida : horario encerrado\n(pos-evento)
    Confirmada --> Confirmada : falha de sync Calendar\n(worker retry/backoff, ADR-002)\n[estado NAO muda]

    Cancelada --> [*]
    Concluida --> [*]

    note right of Confirmada
        Sistema e fonte de verdade (ADR-003).
        Calendar e espelho; falha de sync
        nao reverte a confirmacao (EC-02).
        Sync propagado em ate 2 min (NFR-02).
    end note
```

---

## Caminho de Discovery
...
## Fontes e registro de provenencia

Esta secao consolida os blocos de provenencia de cada secao do documento, relocados do corpo para preservar a rastreabilidade sem poluir a leitura.
...
- **Confianca:** 76
- **Origem:** cto_authored
- **Fonte:** RP §5/§8 (NFR-02/06/08) + classificacao tecnica (Workspace/OAuth a Discovery)
- **Situacao:** respondida
- **Disposicao:** decided
...
- **Situacao:** respondida
- **Disposicao:** decided (nao bloqueante; CTO aceitou as decisoes make-or-buy)
- **Observacao:** "Reutilizar Calendar API" condicionado a TA-Q01/Q02/Q04 (DISC-001). Secao nao bloqueante.
...
- **Confianca:** 72
- **Origem:** cto_authored
- **Fonte:** RP §5/§6/§8/§9 (escopo, RN-01/05, recorrencia, EC-01) + classificacao (Hibrido, KB a Discovery)
- **Situacao:** respondida
- **Disposicao:** decided
...
- **Situacao:** rascunho
- **Disposicao:** discovery
- **Observacao:** DISC-001 destrava current-state (min-conf 75) e os pontos (a)/(d) do TA LEVE; NFR-02/06/08 e ADR-006 sao contingentes ao seu fechamento. Responsavel pelo prazo fechado: PO; investigacao: CTO.
